...
use @sqlite3_column_text[Pointer[U8] tag]( handle: Pointer[_PPstmt] tag,
	column: U32 )
use @sqlite3_column_int[U64]( handle: Pointer[_PPstmt] tag, column: U32 )
...
use @sqlite3_bind_blob[U32]( stmt: Pointer[_PPstmt] tag,column: U32,
	data: Pointer[U8] tag, size: USize )
use @sqlite3_bind_int[U32]( stmt: Pointer[_PPstmt] tag, column: U32, data: U64 )
...
class SqliteStmt
  ...
  fun ref text( column: U32 ): String =>
  ...
  fun ref blob( column: U32 ): Array[U8] =>
    ...
    if size > 0 then
      ...
      Array[U8].from_cstring( @noop(ptr), size.usize() )		
    else
      Array[U8].create()
    end
